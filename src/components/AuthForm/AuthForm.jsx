...
const AuthForm = ({ isSignUp }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const user = !isSignUp
        ? await signIn({ login: formData.login.trim(), password: formData.password.trim() })
        : await signUp({ name: formData.name.trim(), login: formData.login.trim(), password: formData.password.trim() });
...
//   return (
//     <SWrapper themeMode={theme}>
//       <SContainerSignIn themeMode={theme}>
//         <SModal themeMode={theme}>
//           <SModalBlock themeMode={theme}>
//             <SModalTtl>
//               <h2>{isSignUp ? "Регистрация" : "Вход"}</h2>
//             </SModalTtl>
...
//       return "Введите email";
//     }
//     if (!/\S+@\S+\.\S+/.test(formData.login.trim())) {
...
//       if (user) {
//         // сохраняем унифицированные данные пользователя
//         updateUserInfo({
//           name: user.name,
//           email: user.email || user.login,
